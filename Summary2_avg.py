...
if __name__ == "__main__":
    ...
    #similarity quartiles for means
    q_ind = [0.29, 0.32]
    ...
    proto = ['Most of the stocks of ',' sector, has been on average ','to the most virtuous stocks of ', ' sector, for ', ' indicator.' ]
            ...
            if sect1!=sect2:
                ...
                for indd in dfs[sect1][sect2].keys():
                    if dfs[sect1][sect2][indd].mean(axis=1).size>0:
                        ...
                        rs[sect1][sect2][indd][0] = dfs[sect1][sect2][indd].mean(axis=0)[dfs[sect1][sect2][indd].mean(axis=0)<q_ind[0]].size / dfs[sect1][sect2][indd].mean(axis=0).size
                        rs[sect1][sect2][indd][1] = dfs[sect1][sect2][indd].mean(axis=0)[(dfs[sect1][sect2][indd].mean(axis=0)>q_ind[0]) & (dfs[sect1][sect2][indd].mean(axis=0)<q_ind[1])].size / dfs[sect1][sect2][indd].mean(axis=0).size
                        rs[sect1][sect2][indd][2] = dfs[sect1][sect2][indd].mean(axis=0)[dfs[sect1][sect2][indd].mean(axis=0)>q_ind[1]].size / dfs[sect1][sect2][indd].mean(axis=0).size
            ...
            if sect1!=sect2:
                for ind in indexes:
                    if ind in rs[sect1][sect2].keys():
                        ...
                        if dfs[sect1][sect2][ind].mean(axis=1).size>0:
                            m = dfs[sect1][sect2][ind].mean(axis=0).mean()
                            ress = 0
                            if m < q_ind[0]:
                                summ = proto[0]+sect1+proto[1]+summa[0]+proto[2]+sect2+proto[3]+ind+proto[4]
                                ress =0
                            if m < q_ind[1] and m > q_ind[0]:
                                summ = proto[0]+sect1+proto[1]+summa[1]+proto[2]+sect2+proto[3]+ind+proto[4]
                                ress =1
                            if m > q_ind[1]:
                                summ = proto[0]+sect1+proto[1]+summa[2]+proto[2]+sect2+proto[3]+ind+proto[4]
                                ress =2
    ...
    to_exc.to_excel("summary2_avg.xlsx")
